...
def generate_assertion(sf_root_url, user_id, client_id,audience):
    ...
    not_valid_before = issue_instant - timedelta(minutes=5)
    not_valid_after = issue_instant + timedelta(minutes=5)
...
def sign_assertion(xml_string):
    key = xmlsec.Key.from_file("/Users/macbug/Desktop/<PRIVATE KEY>.pem", xmlsec.KeyFormat.PEM)
...
if __name__ == '__main__':
    unsigned_assertion = generate_assertion("<SAML ENDPOINT>", "<EMAIL ADDRESS>", "<HOSTNAME>", "<TOKEN URL>")
    print(sign_assertion(unsigned_assertion))
